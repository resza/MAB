...
void Blowfish_EncryptChar(BLOWFISH_CTX *ctx, unsigned char *xx)
{
	...
	for (i=3;i>=0;i--)
	{
		...
		hex_dump("xl",&xl, 4);
		// 4 Byte Pertama
		xr = xr + xx[i+4] << (i*8);			// 4 Byte Kedua
	}
#endif
	xl = (unsigned long) ((xx[0] << 24) & 0xff000000);
	xl = xl + (unsigned long) ((xx[1] << 16) & 0x00ff0000);
	xl = xl + (unsigned long) ((xx[2] << 8) & 0x0000ff00);
	xl = xl + (unsigned long) ((xx[3] ) & 0x000000ff);

	xr = (unsigned long) ((xx[4] << 24) & 0xff000000);
	xr = xr + (unsigned long) ((xx[5] << 16) & 0x00ff0000);
	xr = xr + (unsigned long) ((xx[6] << 8) & 0x0000ff00);
	xr = xr + (unsigned long) ((xx[7] ) & 0x000000ff);

	Blowfish_Encrypt(ctx, &xl, &xr);

	xx[0]= (unsigned char)((xl >> 24)& 0x000000FF) ;
	xx[1]= (unsigned char)((xl >> 16)& 0x000000FF) ;
	xx[2]= (unsigned char)((xl >> 8)& 0x000000FF) ;
	xx[3]= (unsigned char)(xl & 0x000000FF) ;
	
	xx[7]= (unsigned char)(xr & 0x000000FF) ;
	xx[6]= (unsigned char)((xr >> 8)& 0x000000FF) ;
	xx[5]= (unsigned char)((xr >> 16)& 0x000000FF) ;
	xx[4]= (unsigned char)((xr >> 24)& 0x000000FF) ;
	...
	for(i =0;i<8;i++)
	{
		if (i<=3)
		{
			xx[i] = (unsigned char)(xl >> (i*8)& 0x000000FF) ;
		}
		else
		{	
			xx[i] = (unsigned char)(xr >> ((4-i)*8)& 0x000000FF) ;
		}
...
// encrypt + additiona message length
// return with addtional length
word Blowfish_EncryptMsg(unsigned char *keys, const byte * msg, word lenmsg,int channel)
{
   	unsigned char dc[1024];    //message
   	word lendc;
   	word loopdc;
	short i;
	unsigned char masking[8];

	BLOWFISH_CTX  ctx;

	Blowfish_Init(&ctx, keys, 8);
	
	memset(dc, 0, sizeof(dc));
	memcpy(dc+2, msg, lenmsg);
	...
	case 2:
		...
		loopdc = ((lenmsg-2)+7)/8;
		lendc = loopdc * 8;		// msg to be encrypted
		...
		for(i=0;i<loopdc;i++)
		{
			XORall(dc+4+(i*8), masking, dc+4+(i*8));
			Blowfish_EncryptChar(&ctx, dc+4+(i*8) );
			memcpy(masking, dc+4+(i*8),8);
		}
		...
		dc[1] += lendc %10;	
		 ...
		 //hex_dump_char("Hasil encrypt", dc, lendc); 

		memcpy((char *)msg,dc,lendc+2);
		return lendc+2;
...
//		hex_dump("before enc", dc, lendc+2);
		memset(masking,0,8);
		
		for(i=0;i<loopdc;i++)
		{
			XORall(dc+2+(i*8), masking, dc+2+(i*8));
			Blowfish_EncryptChar(&ctx, dc+2+(i*8) );
			memcpy(masking, dc+2+(i*8),8);
		}

	       //hex_dump_char("Hasil encrypt", dc, lendc); 

		memcpy((char *)msg,dc,lendc);
		return lendc;
...
{
   	unsigned char decry[1024];    //message
   	word lendc;
   	word loopdc;
	short i;
	unsigned char masking[8];
	unsigned char prevchar[8];
	...
	case 2:
		...
		//hex_dump("message before decry", msg, lenmsg);
		//prtS("2");
		lendc = ((word) msg[2])*256;
		lendc += ((word) msg[3]) ;

		memset(decry, 0, sizeof(decry));
		memcpy(decry , msg, lenmsg);	//122

	       //hex_dump_char("message before decry 2", decry, lenmsg);

		lenmsg = lendc;		// len asli 116
		...
		lendc = loopdc * 8;		// llen msg to be encrypted 120

//		memset(decry, 0, sizeof(decry));

		// termasuk message length byte(2) 122
		...
		for(i=0;i<loopdc;i++)
		{
			memcpy(prevchar,decry+4+i*8 ,8);
			Blowfish_DecryptChar(&ctx, decry+4+i*8 );
			XORall(decry+4+(i*8), masking, decry+4+(i*8));	
			memcpy(masking, prevchar,8);
		}
	
	//hex_dump_char("Response Message", decry+4, lendc);
		
		memcpy((char *)msg,decry+2,lenmsg+2);
	//	memcpy(msg,decry+4,lenmsg);
	...
	//	lendc = ((word) msg[0])*100;
	//	lendc += (((word) msg[1]) >> 4) *10;
	//	lendc += (((word) msg[1]) & 0x0f);

		//hex_dump("message before decry", msg, lendc);
	lendc = ((word) msg[0])*256;
		lendc += ((word) msg[1]) ;
		...
		for(i=0;i<loopdc;i++)
		{
			memcpy(prevchar,decry+2+i*8 ,8);
			Blowfish_DecryptChar(&ctx, decry+2+i*8 );
			XORall(decry+2+(i*8), masking, decry+2+(i*8));	
			memcpy(masking, prevchar,8);
		}
	
		memcpy((char *)msg,decry+2,lenmsg);
		//hex_dump("dec", msg, lenmsg);
		return lenmsg;
